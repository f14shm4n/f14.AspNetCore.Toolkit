...
namespace f14.AspNetCore.EntityFrameworkCore.Repositories
{
    ...
    public interface IAsyncRepository<T> where T : class
    {
        ...
        /// <returns>The async task which returns the <see cref="List{T}"/> of entities or null.</returns>
        Task<List<T>?> GetAllAsync(Expression<Func<T, bool>> filter, int skip, int take, CancellationToken cancellationToken = default);
