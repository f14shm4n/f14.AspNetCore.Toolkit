...
namespace f14.AspNetCore.Identity
{
    ...
    public class UserUpdater<TUser, TUserInfo> : IdentityUpdater
        where TUser : class
        where TUserInfo : UserInfo
    {
        ...
        public virtual async Task UpdateAsync(UserManager<TUser> userManager, IEnumerable<TUserInfo> users, Func<TUserInfo, TUser> userFactory)
        {
        ...
        /// <returns>An asynchronous operation.</returns>
        private async Task UpdateRolesAsync(UserManager<TUser> userManager, TUser user, IEnumerable<string> roles)
        {
